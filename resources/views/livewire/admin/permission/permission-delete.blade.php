<div>
    <button type="button" data-tooltip-target="delete-{{ $permission->id }}" wire:click="openModal"
        class="flex items-center justify-center text-red-500 hover:text-red-700 font-bold">
        Delete
    </button>

    <div id="delete-{{ $permission->id }}" role="tooltip"
        class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
        Edit Permissions {{ $permission->name }}
